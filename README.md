...
定时任务会先检查当天是否已经生成过简报，已生成则跳过，避免重复推送。手动 `Run workflow` 会绕过这个去重检查，方便测试 DeepSeek 和推送链路。
...
进入仓库的 `Actions` 页面，选择 `AI Application Brief`，点击 `Run workflow`。手动运行会强制生成并推送一份新的简报。
